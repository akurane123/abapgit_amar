FUNCTION zrf_pick_hu_close_check.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  CHANGING
...
*         First Check if Material is in the HU
          ...
          IF sy-subrc IS NOT INITIAL AND
            ...
            MESSAGE e325 WITH ordim_confirm-matnr lv_huident.
          ENDIF.
...
********************************ATTP**********************************
  ...
  DATA: ls_objdata TYPE /sttpec/s_att_obj_response,
        lv_meinh   TYPE marm-meinh.


  IF ordim_confirm-vlenr IS INITIAL.

    IF lv_field = '/SCWM/S_RF_ORDIM_CONFIRM-BATCH_VERIF'.
...
*Check if serial number exist in the ATTP
    IF lv_field = 'ZRF_S_CHILD_SN-RFSN'.

* check if SGTIN is already scanned
      ASSIGN zt_child_sn[ rfsn = zs_child_sn-rfsn ] TO FIELD-SYMBOL(<fs_child_sn>).
      IF sy-subrc = 0.
* Error Message
      ENDIF.
      PERFORM validate_sn USING zs_child_sn-rfsn
                          CHANGING ls_objdata
                                   lv_meinh.
      IF lv_meinh =  ordim_confirm-altme.
        zs_child_sn-uom = lv_meinh.
        zs_child_sn-rfhu_parent1 = ordim_confirm-pickhu.
        SHIFT zs_child_sn-rfhu_parent1 LEFT DELETING LEADING '0'.
        CONCATENATE '(00)' zs_child_sn-rfhu_parent1 INTO zs_child_sn-rfhu_parent1.
        APPEND zs_child_sn TO zt_child_sn.
      ENDIF.

      DESCRIBE TABLE zt_child_sn LINES DATA(lv_scan).
      zs_child_sn-count =  lv_scan.
      IF ordim_confirm-vsola_chr = lv_scan.
        /scwm/cl_rf_bll_srvc=>set_field( '/SCWM/S_RF_ORDIM_CONFIRM-PICKHU_VERIF' ).
        ordim_confirm-nista_verif = lv_scan.
        /scwm/cl_rf_bll_srvc=>set_screlm_input_off( 'ZRF_S_CHILD_SN-RFSN' ).
        CLEAR: zs_child_sn-rfsn.
      ELSEIF ordim_confirm-vsola_chr > lv_scan.
        /scwm/cl_rf_bll_srvc=>set_field( 'ZRF_S_CHILD_SN-RFSN' ).
        ordim_confirm-nista_verif = lv_scan.
        CLEAR: zs_child_sn-rfsn.
      ENDIF.
...
*check if logpos_ext-setting is "hidden"
    DATA(ls_twho_dd) = lo_dd_picking->get_dd_settings( is_who = who ).
